...
/*   Updated: 2020/12/20 23:49:45 by agiraude         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	skybox_render(t_scene *sc, int x, t_ray *ray)
{
	...
	while (y < ray->draw_start)
	{
		...
		pixel_put_buffer(sc, x, y, tex_get_color(&sc->texs[TEX_BG], tex_x, y * tex_y));
		y++;
	}
}
...
void	ground_render(t_scene *sc)
{
	...
	while (y < sc->set.proj.hg)
	{
		...
		floor.step_x = floor.row_dist * (floor.ray_dir_x1 - floor.ray_dir_x0) / sc->set.proj.wd;
		floor.step_y = floor.row_dist * (floor.ray_dir_y1 - floor.ray_dir_y0) / sc->set.proj.wd;
		...
		while (x < sc->set.proj.wd)
		{
			...
			floor.tex_x = (int)(floor.tex->wd * (floor.x - floor.cell_x)) & (floor.tex->wd -1);
			floor.tex_y = (int)(floor.tex->hg * (floor.y - floor.cell_y)) & (floor.tex->hg -1);

			floor.x += floor.step_x;
			floor.y += floor.step_y;

			pixel_put_buffer(sc, x, y, tex_get_color(floor.tex, floor.tex_x, floor.tex_y));
			x++;
		}
		y++;
	}
